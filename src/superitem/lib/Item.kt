...
import superitem.lib.SIManager.features
import kotlin.reflect.KClass

typealias Item = IContentScript
val Item.clsName: String get() = javaClass.simpleName
fun <T : Feature<out Any>> Item.require(feature: T): T{
    feature.bind(this)
    features.get().getOrPut(feature::class,::mutableListOf).add(feature)
    return feature
}
//判断是否有对应feature
inline fun <reified T:Feature<*>> Item.has() = !features.get()[T::class].isNullOrEmpty()
/**
 * 安全的获取指定类型的feature
 * @param index feature序数(如果有多个,从0开始)
 * @exception RuntimeException 如果不存在指定类型的feature
 */
inline fun <reified T:Feature<*>> Item.get(index: Int=0):T = features.get()[T::class]?.getOrNull(index) as? T
